...
mel_ornaments <- list.files('ornament_analysis/ornament_images', 'mel_.', full.names = TRUE) %>%
...
walk(mel_ornaments, highlight)

get_ornament_values <- function(image_file, ornaments) {
  # image_file <- sample(mel_ims, 1); ornaments <- mel_ornaments
...
mel_ims <- list.files('data/melanic_coloration_warped_v2', full.names = TRUE, recursive = TRUE) %>%
  # remove dumb Apple icon file
  str_subset('Icon', negate = TRUE) %>%
  setNames(., basename(.) %>% tools::file_path_sans_ext())

plan(multisession, workers = 16)
mel_out <- future_map_dfr(
  mel_ims, get_ornament_values, mel_ornaments,
...
write_rds(mel_out, 'ornament_analysis/mel_ornaments.rds')
...
if (FALSE) {
  ...
  mel_pixsets <- list.files('ornament_analysis/ornament_images', 'mel_.\\.png', full.names = TRUE) %>%
    map(\(.x) {
    ...
    setNames(paste0('mel_', seq_along(.)))
  all_mel_ornaments <- reduce(mel_pixsets, `+`, .init = bg)
  all_mel_ornaments <- recolor_image(
    all_mel_ornaments,
    list(c(1, 0, 0), c(0, 0, 0)), list(c(1e-9, 0, 0), 'grey70')
  )
  plot(all_mel_ornaments)
  save.image(all_mel_ornaments, 'ornament_analysis/visualization/mel_ornaments_abstract.png')
  mel_ornaments <- map(mel_pixsets, ~bg + .x)
  mel_ornaments <- map(
    mel_ornaments, recolor_image,
    list(c(1, 0, 0), c(0, 0, 0)), list(c(1e-9, 0, 0), 'grey70')
  )
  fn <- paste0('ornament_analysis/ornament_minis/', names(mel_ornaments), '.png')
  walk2(mel_ornaments, fn, ~save.image(.x, .y))
  labs <- paste0("<img src='", fn, "'width='75' />") %>% setNames(paste0('mel_', seq_along(fn)))
  labs_large <- paste0("<img src='", fn, "'width='125' />") %>% setNames(paste0('mel_', seq_along(fn)))

  # summary of the ornaments
  mel_out %>% group_by(ornament) %>% summarise(across(pixels_present:present, mean))
  ...
  # panels <- expand_grid(x = names(mel_out_wide)[-1], y = names(mel_out_wide)[-1])
  # plots <- list()
  # for (i in seq_len(nrow(panels))) {
  #   plots[[i]] <- ggplot(
  #     mel_out_wide, aes(x = .data[[panels$x[i]]], fill = factor(.data[[panels$y[i]]]))
  #   ) + geom_bar(show.legend = FALSE, position = 'fill') + theme_void()
  # }
  # cowplot::plot_grid(plotlist = plots, byrow = FALSE)

  # for (i in seq_len(nrow(panels))) {
  #   chisq.test(mel_out_wide[[panels$x[[i]]]], mel_out_wide[[panels$y[i]]]) %>% print()
  # }
  ...
  ggsave('ornament_analysis/visualization/mel_ornament_response_to_selection.png', w = 8, h = 4, scale = 1.2)
  ...
  ggsave('ornament_analysis/visualization/mel_ornament_response_to_selection2.png', w = 8, h = 4, scale = 1.2)
}
