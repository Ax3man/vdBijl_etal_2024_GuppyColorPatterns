...
cat reference/scaffolds.list | parallel -j 44 \
    "$bcf filter gwas2/scaffold_vcfs/{}.vcf.gz -Oz -o gwas2/scaffold_vcfs_filtered/{}.vcf.gz \
...
echo "### bcf concat 1"
$bcf concat -Oz --threads 46 gwas2/scaffold_vcfs_filtered/*.vcf.gz > gwas2/filtered.vcf.gz

# Make a vcf of just the unplaced scaffolds, we'll use that in our GWAS instead of running GEMMA on each scaffold
echo '### bcf concat 2'
$bcf concat -Oz --threads 46 gwas2/scaffold_vcfs_filtered/NW*.vcf.gz > gwas2/Un.vcf.gz
...
echo '### bcf view'
$bcf view --targets-file reference/autosomes.list --threads 46 gwas2/filtered.vcf.gz -Oz -o gwas2/filtered_autosomes.vcf.gz
