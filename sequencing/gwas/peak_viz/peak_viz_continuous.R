peak_viz_continuous <- function(
  ...
  gwas <- get_gwas_results(trait, chr, pos, viz_range, fdr.level)
  geno <- get_genotypes(vcf, chr, viz_range)
  region_cov <- get_region_coverage(chr, pos, viz_range)
  linkage <- get_linkage(geno, trait, workers = 12)

  genes <- make_gene_panel(chr, viz_range)
  ...
  depth_yuying <- depth + aes(color = 'male') +
    ...
    scale_color_manual(values = c(female = 'firebrick', male = 'black'), name = NULL)
  ...
  ornament_effect <- make_ornament_panel(ornament, geno, chr, pos)
  allelic_depth <- make_allelic_depth_panel(geno, chr, pos)
  ...
  if (!is.null(file)) {
    ggsave(file, pl, width = 12, height = 12)
  } else {
    return(pl)
  }
  ggsave(file, width = 7, height = 12)
