# Selection decision script for Replicate 3, third selection (on the second offspring, F2).
library(tidyverse); theme_set(theme_bw())

photo_database <- data.table::fread('photo_database.csv') %>%
  as_tibble() %>%
  filter(replicate == 'replicate_3', generation == 'gen_3')
...
fathers <- data.table::fread('selection_decisions/finished/rep3_gen2_previous_decisions.csv') %>%
  select(fish_id, selection)
previous_decisions <- data.table::fread('selection_decisions/rep3_gen3_previous_decisions.csv')
...
down <- filter(pics_summ, selection == 'down_selected') %>%
  # We lost m3-437, so need to remove him here
  filter(!c(fish_id %in% c('m3-437', 'm3-439'))) %>%
...
data.table::fwrite(up, 'selection_decisions/rep3_gen3_up.csv')
data.table::fwrite(down, 'selection_decisions/rep3_gen3_down.csv')
data.table::fwrite(decisions, 'selection_decisions/rep3_gen3_previous_decisions.csv')
latest_decisions %>%
  select(date, fish_id, car_perc, decisions) %>%
  mutate(car_perc = round(car_perc, digits = 2)) %>%
  arrange(date, fish_id) %>%
  data.table::fwrite('selection_decisions/rep3_gen3_latest_decisions.csv')
