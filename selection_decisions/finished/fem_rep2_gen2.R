...
# Load parental gen data
parental_gen <- bind_rows(
  data.table::fread('selection_decisions/finished/rep1_parentalgen.csv'),
  data.table::fread('selection_decisions/finished/rep2_parentalgen.csv')
) %>%
...
d <- data.table::fread('photo_database.csv') %>%
  ...
  # Finally, add the selection type, only for rep1 & rep2 gen2 right now.
  left_join(parental_gen, by = c('sire' = 'fish_id'))
...
model_data <- d %>%
  filter(replicate %in% c('replicate_1', 'replicate_2')) %>%
...
m <- brm(
  ...
  backend = 'cmdstanr', chains = 10, cores = 10, warmup = 1000, iter = 5000,
  control = list(adapt_delta = 0.99, max_treedepth = 10),
  file = 'selection_decisions/models/rep2_gen2.rds'
)
...
data.table::fwrite(up_selected_females, 'selection_decisions/fem_rep2_gen2_up.csv')
data.table::fwrite(down_selected_females, 'selection_decisions/fem_rep2_gen2_down.csv')
