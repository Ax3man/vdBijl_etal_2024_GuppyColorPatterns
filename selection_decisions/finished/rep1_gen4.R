# Selection decision script for Replicate 1, selection to sequence (on the third offspring, F3).
...
photo_database <- data.table::fread('photo_database.csv') %>%
  as_tibble() %>%
  filter(replicate == 'replicate_1', generation == 'gen_4')
...
fathers <- data.table::fread('selection_decisions/finished/rep1_gen3_previous_decisions.csv') %>%
  select(fish_id, selection)
previous_decisions <- data.table::fread('selection_decisions/rep1_gen4_previous_decisions.csv')
...
to_seq_up <- picking_rounds(
  ...
  left_join(to_seq_up_limited, by = c("date", "fish_id", "car_perc", "sire", "dam", "date_of_birth", "selection")) %>%
  mutate(sample_priority = coalesce(sample_priority, 'backup sample'))
stopifnot(sum(to_seq_up$sample_priority == 'first choice') == 50)

to_seq_down_limited <- picking_rounds(
  filter(pics_summ, selection == 'down_selected'),
  desc(car_perc),
  50
) %>% mutate(sample_priority = 'first choice')
to_seq_down <- picking_rounds(
  ...
  left_join(to_seq_down_limited, by = c("date", "fish_id", "car_perc", "sire", "dam", "date_of_birth", "selection")) %>%
  mutate(sample_priority = coalesce(sample_priority, 'backup sample'))
...
down_stock <- filter(pics_summ, selection == 'down_selected') %>%
  # remove fish selected for sequencing
...
data.table::fwrite(to_seq_up, 'selection_decisions/rep1_gen4_up_seq.csv')
data.table::fwrite(to_seq_down, 'selection_decisions/rep1_gen4_down_seq.csv')
data.table::fwrite(up_stock, 'selection_decisions/rep1_gen4_up_stock.csv')
data.table::fwrite(down_stock, 'selection_decisions/rep1_gen4_down_stock.csv')

data.table::fwrite(decisions, 'selection_decisions/rep1_gen4_previous_decisions.csv')
latest_decisions %>%
  select(date, fish_id, selection, car_perc, decisions) %>%
  mutate(car_perc = round(car_perc, digits = 2)) %>%
  arrange(date, fish_id) %>%
  data.table::fwrite('selection_decisions/rep1_gen4_latest_decisions.csv')
