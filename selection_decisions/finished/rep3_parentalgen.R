# Selection decision script for Replicate 3, first selection (on parents from the stock tanks).
library(tidyverse)
library(imager)

photo_database <- data.table::fread('photo_database.csv') %>%
  as_tibble() %>%
  filter(replicate == 'replicate_3', generation == 'parental_gen_1')
...
View(pics_summ)
data.table::fwrite(pics_summ, 'selection_decisions/rep3_parentalgen.csv')
...
ggplot(photo_database, aes(seq_along(unique_id), as.numeric(manual_landmarks))) +
  ...
  theme_minimal() +
  labs(x = 'photo', y = 'percent manual fish extraction')
ggplot(photo_database, aes(seq_along(unique_id), as.numeric(file.exists(paste0('data/carotenoid_coloration_manual/', unique_id, '.png'))))) +
  geom_point(shape = '|') +
  geom_smooth(method = 'glm', method.args = list(family = 'binomial'), color = 'firebrick') +
  scale_y_continuous(expand = c(0, 0.01), labels = scales::percent) +
  theme_minimal() +
  labs(x = 'photo', y = 'percent manual carotenoid extraction')
