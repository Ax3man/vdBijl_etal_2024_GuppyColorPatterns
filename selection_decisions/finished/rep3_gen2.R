# Selection decision script for Replicate 3, second selection (on the first offspring, F1).
library(tidyverse)

photo_database <- data.table::fread('photo_database.csv') %>%
  as_tibble() %>%
  filter(replicate == 'replicate_3', generation == 'gen_2')
photo_database %>% group_by(fish_id) %>%
  filter(!(fish_id %in% c('m3-208', 'm3-247', 'm3-258')), n() != 4)
photo_database %>% group_by(fish_id, facing_direction) %>%
  filter(!(fish_id %in% c('m3-208', 'm3-247', 'm3-258')),n() != 2)

pedigree <- data.table::fread('data/pedigree.csv') %>%
  mutate(across(everything(), tolower))
fathers <- data.table::fread('selection_decisions/finished/rep3_parentalgen.csv') %>%
  select(fish_id, selection)
previous_decisions <- data.table::fread('selection_decisions/rep3_gen2_previous_decisions.csv')
...
data.table::fwrite(up, 'selection_decisions/rep3_gen2_up.csv')
data.table::fwrite(down, 'selection_decisions/rep3_gen2_down.csv')
data.table::fwrite(decisions, 'selection_decisions/rep3_gen2_previous_decisions.csv')
latest_decisions %>%
  select(date, fish_id, car_perc, decisions) %>%
  mutate(car_perc = round(car_perc, digits = 2)) %>%
  arrange(date, fish_id) %>%
  data.table::fwrite('selection_decisions/rep3_gen2_latest_decisions.csv')
