...
d <- data.table::fread('photo_database.csv') %>%
  ...
  mutate(selection = coalesce(selection, selection2)) %>% select(-selection2)

# Make a new data.frame with representative females
representative_females <- d %>%
  filter(replicate == 'replicate_3', generation == 'gen_2') %>%
  group_by(dam, sire) %>%
  slice(1) %>%
  ungroup() %>%
  mutate(
...
m <- brm(
  ...
  backend = 'cmdstanr', chains = 14, cores = 14, warmup = 500, iter = 5000, thin = 10,
  control = list(adapt_delta = 0.995, max_treedepth = 12),
  file = 'selection_decisions/models/rep3_gen2.rds'
)
# Mean chain execution time: 137626.5 seconds.
# Total execution time: 139012.3 seconds.
summary(m)
pp_check(m, nd = 50)
...
filter(VC_df, draw %in% sample(unique(draw), size = 1000)) %>%
  mutate(
    param = factor(param, c('fish_idA' ,'fish_idX', 'patriline', 'grow_tank', 'fish_id', 'residual__'))
  ) %>%
  group_by(draw) %>%
  mutate(sorting_value = sum(value[param %in% c('fish_idX', 'fish_idA', 'patriline')])) %>%
  ungroup() %>%
  mutate(draw_sorted = fct_reorder(factor(draw), sorting_value)) %>%
  ggplot(aes(draw_sorted, value, fill = param)) +
  geom_col(position = 'fill', width = 1) +
  scale_fill_manual(
    limits = c('fish_idA' ,'fish_idX', 'patriline', 'grow_tank', 'fish_id', 'residual__'),
    labels = c('Autosome', 'X', 'Y', 'Grow\ntank', 'Environment\n(other)', 'Measurement\nerror'),
    values = c('firebrick', 'navy', 'darkmagenta', 'grey80', 'grey50', 'grey20')
  )
...
data.table::fwrite(up_selected_females, 'selection_decisions/fem_rep3_gen2_up.csv')
data.table::fwrite(down_selected_females, 'selection_decisions/fem_rep3_gen2_down.csv')
