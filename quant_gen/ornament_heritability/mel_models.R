...
mel_out_wide <- pivot_wider(mel_out, unique_id, names_from = ornament, values_from = pixels_present)

# Get the melanin values for each photo
d <- data.table::fread('photo_database.csv') %>%
  ...
  left_join(mel_out_wide, 'unique_id')
...
if (FALSE) { # run interactively only
  ...
  #walk(finished_models, priorsense::powerscale_sensitivity)
  ...
  mel_pixsets <- list.files('ornament_analysis/ornament_images', 'mel*', full.names = TRUE) %>%
    map(\(.x) {
  ...
  ggsave('ornament_analysis/visualization/mel_ornament_h2.png', w = 8, h = 4, scale = 1.2)
}
