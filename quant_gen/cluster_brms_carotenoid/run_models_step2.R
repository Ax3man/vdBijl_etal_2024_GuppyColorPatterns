...
full_models <- data.frame(file = list.files('quant_gen/cluster_brms_carotenoid/posteriors_1/')) %>%
...
fit_second_models <- function(Data, cutoff = 0.01, verbose = FALSE) {
  ...
  stepsize <- closest_models$file %>%
    paste0('quant_gen/cluster_brms_carotenoid/meta_1/', .) %>%
    map(read_rds) %>%
  ...
  params <- closest_models$file %>%
    paste0('quant_gen/cluster_brms_carotenoid/posterior_summaries_1/', .) %>%
  ...
  post_file <- paste0(
    'quant_gen/cluster_brms_carotenoid/posteriors_2/pixel_',
  ...
  stan <- readLines('quant_gen/cluster_brms_carotenoid/model2.stan')
