...
model <- cmdstan_model('quant_gen/cluster_brms_melanin/model.stan')
...
fit_first_models <- function(Data, cutoff = 0.01, verbose = FALSE) {
  ...
  post_file <- paste0(
    'quant_gen/cluster_brms_melanin/posteriors_1/pixel_',
  ...
  if (file.exists(post_file)) return(NULL)
  ...
  fit <- model$sample(
    ...
    iter_sampling = 1500,
    adapt_delta = 0.95,
    step_size = 0.2 # might change this to a good starting value later (should only affect speed)
  )
