...
purrr::walk(
  ...
  \(embed_dim) {
    ...
    output_name <- paste0('mel_model_ped_fullcolor_comparison/embed_dim_', embed_dim)
    if (file.exists(paste0('dimension_reduction/triplet_loss_encoders/weights/', output_name, '.hdf5'))) {
      return(NULL)
    }

    fit_triplet_loss_model(
      img_dir = 'data/melanic_coloration_warped_v2/',
...
if (FALSE) { #only run interactively
  ...
  eval_fun <- function(
    ...
    #weights <- 'dimension_reduction/triplet_loss_encoders/weights/mel_model_ped_fullcolor_comparison/embed_dim_2.hdf5'
    #embed_dim <- 2
  ...
  out <- map2_dfr(
    list.files('dimension_reduction/triplet_loss_encoders/weights/mel_model_ped_fullcolor_comparison', f = TRUE),
    1:6,
    eval_fun,
    .id = 'embed_dim'
  )
  ...
  ggplot(
    ...
    expand_limits(y = 1) +
    labs(x = 'Nr of embedding dimensions', y = 'Accuracy', color = 'Min relatedness') +
    theme_classic()
}
