...
snp_inheritance <- bind_rows(
  data.table::fread('sequencing/gwas/snp_inheritance/gwas_snp_inheritance/black_ornaments.csv'),
  data.table::fread('sequencing/gwas/snp_inheritance/gwas_snp_inheritance/mel_PIE.csv')
) %>%
...
make_upset <- function(d, min_size) {
  u <-  upset(
    as.data.frame(d),
    intersect = rev(c('pattern', paste0('B', 1:8))),
