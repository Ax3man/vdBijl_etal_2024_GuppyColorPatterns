...
carotenoid_extractor <- unserialize_model(
  read_rds('../color_analyses/keras_carotenoid_detection/unet.rds'),
  custom_objects = list(dice_loss = dice_loss, dice_coef = dice_coef)
)

extract_carotenoid <- function(extracted_fish) {
  ...
  p <- predict(carotenoid_extractor, X)
