...
car_out <- read_rds('ornament_analysis/car_ornaments.rds') %>%
  ...
  left_join(dplyr::select(photo_database, fish_id, unique_id), 'unique_id') %>%
  mutate(fish_id = toupper(fish_id)) %>%
  left_join(dplyr::select(selection, fish_id, replicate, generation, selection), 'fish_id')
...
pd3 <- car_out %>%
  ...
  summarise(present_10 = Mode(present_10),
            ornament_size = mean(pixels_present),
            .groups = 'drop_last'
  ) %>%
  mutate(selection = ifelse(generation == 'P', 'stock', selection))

P_car_orn_pres <- ggplot(pd3, aes(generation, present_10, fill = selection)) +
