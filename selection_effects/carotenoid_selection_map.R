...
df <- future_map_dfr(
  ...
  ~load.image(.x) %>%
    ...
    mutate(carotenoid = as.numeric(c.4 > 0.5)) %>%
    select(x, y, carotenoid) %>%
...
df <- df %>%
  group_by(x, y) %>%
  filter(mean(carotenoid) > 0.01) %>%
  ungroup() %>%
  inner_join(photo_database, by = c('image' = 'unique_id')) %>%
  inner_join(selection, by = c('fish_id', 'replicate', 'generation')) %>%
  select(x, y, replicate, generation, selection, image, fish_id, facing_direction, carotenoid)
...
selection_effect <- function(Data, verbose = FALSE) {
  ...
  data_small <- Data %>%
    ...
    summarize(carotenoid = Mode(carotenoid), .groups = 'drop')
  m <- suppressMessages(suppressWarnings(
    glmer(
      carotenoid ~ 1 + selection * generation + (1 | generation:replicate),
      data_small, 'binomial'
    )
  ))
...
write_rds(model_results, 'visualization/per_pixel_models/car_selection_model_results.rds')
